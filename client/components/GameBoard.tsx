...
export default function GameBoard({
  ...
  const render = useCallback(() => {
    ...
    fenceRef.current.forEach((k) => {
      ...
      ctx.beginPath();
      ctx.roundRect(px + 1.5, py + 1.5, CELL - 3, CELL - 3, 3);
      ctx.fill();
    });

    // 3. 普通红苹果 (极简圆润 + 2px 翠绿微叶)
    ...
    ctx.beginPath();
    ctx.arc(fx + 2, fy - 5, 2.2, 0, Math.PI * 2);
    ctx.fill();

    // 3.5 金色幸运果实 (限时奖励 +30 分)
    if (bonusRef?.current) {
    ...
    <div className="bg-white border border-[#E2E8F0] p-4 sm:p-5 rounded-3xl shadow-sm flex flex-col items-center select-none">
      ...
      <div className="w-full grid grid-cols-3 gap-2.5 sm:gap-3 mb-3 sm:mb-4 text-center text-xs">
        <div className="bg-[#F8FAFC] border border-[#E2E8F0] py-2 px-1 rounded-2xl relative overflow-hidden">
          ...
          {hasBonus && (
            <span className="absolute top-1 right-1 flex items-center text-[#D97706] font-extrabold text-[10px] animate-pulse">
              <Sparkles size={11} /> +30
            </span>
          )}
        </div>
        ...
        <div className="bg-[#F8FAFC] border border-[#E2E8F0] py-2 px-1 rounded-2xl">
          <span className="text-[#94A3B8]">用时 </span>
        ...
        <div className="bg-[#EBF8FF] border border-[#66CCFF]/30 py-2 px-1 rounded-2xl">
          <span className="text-[#0099FF] font-medium">速度 </span>
      ...
      >
        ...
        {/* 暂停遮罩 */}
        {isPaused && (
          <div className="absolute inset-0 bg-white/85 backdrop-blur-[2px] flex flex-col items-center justify-center text-[#0F172A]">
            <Pause size={28} className="mb-1 text-[#0099FF]" />
            <span className="text-xs font-bold">已暂停 (按 P 键或空格继续)</span>
          </div>
        )}
