...
- 🌙 **全景深空墨蓝夜间模式 (OLED Deep Dark)**：纯矢量超椭圆日/月切换微拟态按钮；全局注入 `#0A0F1D` 深空变量；Header 导航顶栏、控制中心浮层、Canvas 遮罩、虚拟按键与离屏静态背景全面沉浸自适应，告别任何白边与刺眼反差。
...
- 🏆 **风云榜单行耗时与战绩走势微排版**：
  - 得分与最佳耗时严格保持单行同行紧凑呈现（如 `480 (1m 25s)`），历史无耗时对局优雅回退仅显得分；
  - 底部战况重构为清晰双行结构：首行展示个人最高分与全服超越百分比，次行独立沉淀近 5 局得分走势 SVG 微折线，消除文字与折线挤压踩踏。
- ⏳ **极简外置流光导轨与绝对零物理位移**：
...
- 🛡️ **Level 3 物理重放防作弊与永续凭证**：确定性伪随机种子 (Mulberry32) + Go 后端 1ms 无头物理重放验算真实战绩，辅以无状态 HMAC 防伪令牌与滑动限流；结算端点防御性承接 Bearer Token，保障破纪录时最高分 100% 实时更新入库。
